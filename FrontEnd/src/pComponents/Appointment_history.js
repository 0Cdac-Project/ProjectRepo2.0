...
function AppointmentHistory() {
  ...
  useEffect(() => {
    if (user && user.patientID) {
      axios
        .get(urlHistory + `/by_patient/${user.patientID}`)
        .then((result) => {
          console.log(result);
            ...
            <div className="accordion-item" key={result.appointmentID}>
              <h2 className="accordion-header">
                <button
                  ...
                  data-bs-target={`#panelsStayOpen-collapse-${result.appointmentID}`}
                  aria-expanded="false"
                  aria-controls={`panelsStayOpen-collapse-${result.appointmentID}`}
                >
                  Appointment Id: {result.appointmentID}
                </button>
              </h2>
              <div
                id={`panelsStayOpen-collapse-${result.appointmentID}`}
                ...
                <div className="accordion-body">
                  ...
                  <span>
                    ...
                    <span className="fst-italic">
                      {result.doctor.doctorFirstName}{" "}
                      {result.doctor.doctorLastName}
                    </span>
                  </span>
                  <br />
                  <br />
                  <span>
                    ...
                    <span>{result.appointmentDateTime}</span>
                  </span>
                  <br />
                  <br />
                  <span>
